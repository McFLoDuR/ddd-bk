...
Most Angular applications need to preserve some state so that the same data doesn't need to be fetched time and again from the backend. Examples are information that is needed when switching back from a details view to a list view or information collected during clicking through a wizard.
...
Both hooks get the store passed in as an argument. One more time, by using destructuring, you can focus on a subset of the stores members.
...
Updaters allow the consumer to modify the feature state without actually knowing how it's structured. 
